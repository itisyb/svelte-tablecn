...
describe('/+page.svelte', () => {
	...
	it('should align select cell editor like the original grid', async () => {
		...
		expect(firstItem.className).not.toContain('rounded-none');
	...
	it('should clear selection from the global Escape capture path like the original grid', async () => {
		await render(DataGridCtrlShiftEdgeFixture);
	...
	it('should keep data table range filter unit adornments aligned with original numeric inputs', () => {
		...
		expect(dataTableRangeFilterSource).not.toContain('pointer-events-none absolute inset-y-0');
